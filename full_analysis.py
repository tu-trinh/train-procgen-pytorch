...
if args.plotting:
    ...
    if PLOT_PERF_VS_PROP in args.plots and args.grouped:
        ...
        for metric in include_metrics:
            ...
            axes5.plot(afhp_means[sort_idx], rew_means[sort_idx], color = colors[metric], label = metric if "T" not in metric else skyline_names[metric], linestyle = "dashdot" if "svdd" in metric else ((5, (10, 3)) if metric == "random" else ((0, (3, 5, 1, 5, 1, 5)) if "T" in metric else "solid")))
        ...
        axes5.plot([smallest_afhp, biggest_afhp], [test_perf_mean] * 2, color = "gray", label = f"weak agent in test", linestyle = "dashed")
