...
if args.plotting:
    ...
    if PLOT_PERF_VS_PROP in args.plots and args.grouped:
        ...
        for metric in include_metrics:
            ...
            afhp_means = np.insert(afhp_means, 0, 0)
            rew_means = np.insert(rew_means, 0, test_perf_mean)
            adj_rew_means = np.insert(adj_rew_means, 0, test_perf_mean)
            afhp_means = np.append(afhp_means, 1)
            rew_means = np.append(rew_means, expert_perf_mean)
            adj_rew_means = np.append(adj_rew_means, expert_perf_mean - (10/256 * args.query_cost * 256))
            if "T" in metric:
                print("COORDINATES")
                print("\n".join([str(item) for item in list(zip(afhp_means, rew_means))]))
