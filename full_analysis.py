...
if args.plotting:
    ...
    if PLOT_PERF_VS_PROP in args.plots and args.grouped:
        ...
        for metric in include_metrics:
            ...
            axes6.plot(afhp_means[sort_idx], [train_perf_mean] * len(afhp_means[sort_idx]), color = "black", label = f"weak agent in training")
            axes6.plot(afhp_means[sort_idx], [test_perf_mean] * len(afhp_means[sort_idx]), color = "gray", label = f"weak agent in test")
            axes6.plot(afhp_means[sort_idx], [expert_perf_mean] * len(afhp_means[sort_idx]), color = "tab:brown", label = f"expert agent in test")
